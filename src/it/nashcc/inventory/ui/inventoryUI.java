...
public class inventoryUI extends JFrame implements ActionListener {
	...
	private InventoryController scheduler;
	...
	public inventoryUI() {
		...
		try {			
			scheduler = new InventoryController(getFileName(true));
	...
	private class SchedulerPanel extends JPanel implements ActionListener {
		...
		public SchedulerPanel() {
			...
			tableCatalog.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
				...
				public void valueChanged(ListSelectionEvent e) {
					String name = tableCatalog.getValueAt(tableCatalog.getSelectedRow(), 2).toString();

					Computers c = scheduler.getAssetId(name);
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnAddCourse) {
				...
				if (row == -1) {
					JOptionPane.showMessageDialog(inventoryUI.this, "No course selected in the catalog.");
				} else {
					try {
						if (!scheduler.addComputer(tableCatalog.getValueAt(row, 0).toString(), tableCatalog.getValueAt(row, 1).toString())) {
							JOptionPane.showMessageDialog(inventoryUI.this, "Course doesn't exist.");
						}
					} catch (IllegalArgumentException iae) {
						JOptionPane.showMessageDialog(inventoryUI.this, iae.getMessage());
					}
				}
				updateTables();
				...
				if (row == -1) {
					JOptionPane.showMessageDialog(inventoryUI.this, "No item selected in the schedule.");
				} else {
					scheduler.removeInventory(row);
				}
				updateTables();
				...
				try {
					...
					JOptionPane.showMessageDialog(inventoryUI.this, "Invalid title.");
				}
				...
				try {
					...
					if (eventMeetingDays.length() == 0) {
						JOptionPane.showMessageDialog(inventoryUI.this, "The event must occur on at least one day.");
						return;
					}
					...
					if (hourIdx == -1) {
						JOptionPane.showMessageDialog(inventoryUI.this, "The event times are invalid.");
						return;
					}
					...
					if (weeklyRepeatIdx == -1) {
						JOptionPane.showMessageDialog(inventoryUI.this, "The weekly repeat is invalid.");
						return;
					}
					...
					JOptionPane.showMessageDialog(inventoryUI.this, message);
				}
			}
			
			inventoryUI.this.repaint();
			inventoryUI.this.validate();
		}
		 ...
		 */
		private void updateCourseDetails(Computers c) {
			if (c != null) {
				lblName.setText(c.getName());
				lblSection.setText(c.getModel());
				lblTitle.setText(c.getAssetId());
				lblInstructor.setText(c.getSerialNumber());
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnReviseSchedule) {
				...
				try {
					...
					scheduler.exportInventory(fileName);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(inventoryUI.this, iae.getMessage());
					String fileName = getFileName(false);
					scheduler.exportInventory(fileName);
